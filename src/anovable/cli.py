...
async def main_async(args: argparse.Namespace) -> int:
    ...
    if not mac_address:
        print("Error: No MAC address specified. Use --mac-address or configure in anovable.yaml", file=sys.stderr)
        return 1
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Control Anova Precision Cooker via Bluetooth LE")
    parser.add_argument("--mac-address", help="MAC address of Anova device (overrides config file)")
    parser.add_argument("--config", help="Path to configuration file (default: search standard locations)")
