...
class AnovaConfig:
    ...
    def load_config(self) -> None:
        ...
        try:
            ...
            raise AnovaError(f"Failed to load configuration from {self.config_path}: {e}")
    ...
    def mac_address(self) -> Optional[str]:
        ...
        return self._config.get("anova", {}).get("mac_address")

    @property
    def timeout(self) -> float:
        """Get connection timeout."""
        return self._config.get("anova", {}).get("connection", {}).get("timeout", 5.0)

    @property
    def retry_attempts(self) -> int:
        """Get retry attempts."""
        return self._config.get("anova", {}).get("connection", {}).get("retry_attempts", 3)

    @property
    def default_unit(self) -> str:
        """Get default temperature unit."""
        return self._config.get("anova", {}).get("temperature", {}).get("default_unit", "celsius")

    @property
    def log_level(self) -> str:
        """Get logging level."""
        return self._config.get("anova", {}).get("logging", {}).get("level", "INFO")
